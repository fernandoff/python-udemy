...
class Biblioteca:
    ...
    @property
    def ativo(self):
       return str(Status.ATIVO) if self._ativo else str(Status.INATIVO)
    ...
    def exibir_itens(self):
        ...
        for i, item in enumerate(self._itens, start=1):
            if hasattr(item, "isbn"):
                msg_livro = f"{i}. (Livro) -> Título: {item._titulo} | Autor: {item._autor} | Preço: {item._preco} | ISBN: {item.isbn} | Ativo: {self.ativo}"
                print(msg_livro)
            else: 
                msg_revista = f"{i}. (Revista) -> Título: {item._titulo} | Autor: {item._autor} | Preço: {item._preco} | Edição: {item.edicao} | Ativo: {self.ativo}"
                print(msg_revista)